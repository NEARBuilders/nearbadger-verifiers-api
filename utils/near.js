...
export class NearRPC {
        ...
        async getAccountPubKeys(accountId) {
                return this.getAccountRawKeys(accountId).then(({ result }) =>
                        result?.keys.map((key) => key.public_key),
                );
        }
        ...
        async getCurrentBlockHeight() {
                ...
                return this.send(request).then(
                        (data) => data?.result?.header?.height,
                );
        }
        ...
        async getAccessKeyList(accountId) {
                ...
                return this.send(request).then((data) => data.result);
        }
...
export class Wallet {
        sign(message) {
                ...
                return keyPair.sign(new Uint8Array(Buffer.from(message)));
        }
}
...
export class NearApi {
        async getAccountInfo(accountId) {
                return fetch(`${NEAR_BLOCKS_API}/v1/account/${accountId}`).then(
        ...
        async getAccountTxnInfo(accountId) {
                return fetch(
                        `${NEAR_BLOCKS_API}/v1/account/${accountId}/txns/count`,
                ).then((payload) => payload.json());
        }
...
export class NearAccountInfo {
        ...
        static async getAccountSocialFollowers(accountId) {
                ...
                const accountInfo = await api.getNearSocialFollowInfo(
                        accountId,
                        payload,
                );
        ...
        static async getAccountSocialFollowings(accountId) {
                ...
                const infoCount = Object.keys(
                        accountInfo[accountId]["graph"]["follow"],
                ).length;
                return infoCount;
        }
